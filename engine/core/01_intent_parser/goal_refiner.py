...
class GoalRefiner:
    ...
    def refine(self, project_description: str) -> Dict[str, str]:
        ...
        # Input validation: empty or too short
        if not text:
            return {
                "persona": "未明确",
                "problem": "未明确",
        ...
        # Extract with inferred annotation
        ...
        # Annotate inferred values (fallback was used)
    ...
    @staticmethod
    def _annotate_inferred(value: str, full_text: str, field: str) -> str:
        """Annotate values that were inferred (fallback used) rather than explicitly extracted."""
        if not value:
            return "未明确"
        # Check if the value came from fallback (first sentence) rather than a pattern match
        first_sentence = re.split(r'[。.\n]', full_text)[0].strip().strip('，,。.')
        if value == first_sentence:
            # Was inferred from fallback → check if it's less than 10 chars
            if len(value) < 10:
                return f"{value}（推断）"
        return value
