...
class RejectAnalyzer:
    ...
    def __init__(self, config_path: Optional[str] = None):
        ...
        self.REJECTION_PATTERNS = config.get("rejection_patterns", self.DEFAULT_CONFIG["rejection_patterns"])
        self.CHECKLIST_ITEMS = config.get("checklist_items", self.DEFAULT_CONFIG["checklist_items"])
        self.KEYWORD_MAP = config.get("keyword_map", self.DEFAULT_CONFIG["keyword_map"])
        self.CONFIDENCE_THRESHOLD = config.get("confidence_threshold", self.DEFAULT_CONFIG["confidence_threshold"])
        self.ROOT_CAUSE_DEPTH = config.get("root_cause_depth", self.DEFAULT_CONFIG["root_cause_depth"])
        self.DESCRIPTION_TRUNCATE_LENGTH = config.get("description_truncate_length", self.DEFAULT_CONFIG["description_truncate_length"])
    ...
    def _load_config(self, config_path: Optional[str] = None) -> Dict:
        ...
        try:
            if HAS_CONFIG_LOADER:
                loader = ConfigLoader()
                cfg = loader.get("reject_analyzer")
                if cfg:
                    return cfg
        except Exception:
            pass
    ...
    def _build_root_cause_tree(self, pattern_name: str, description: str) -> Dict[str, str]:
        ...
        """
        truncate_length = self.DESCRIPTION_TRUNCATE_LENGTH
        surface = description[:truncate_length] if len(description) > truncate_length else description
