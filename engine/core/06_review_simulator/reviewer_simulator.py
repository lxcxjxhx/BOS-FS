...
class ReviewerSimulator:
    ...
    def __init__(self, config_path: str = None):
        """Initialize the simulator, loading rules from configuration.

        Args:
            config_path: Optional directory path to JSON config files.
                         If None, uses the bundled defaults directory.
                         If loading fails, falls back to hardcoded defaults.
        """
        self.config = None
        try:
            from engine.config import ConfigLoader
            self.config = ConfigLoader(config_dir=config_path)
            self.config.load_all_defaults()
        except Exception:
            self.config = None
    ...
    def _apply_anti_sycophancy(self, result: dict, project_info: dict, review_type: str) -> None:
        ...
        empty_praise_words = set(
            self.config.get("anti_sycophancy.layer1_lexical.empty_praise_words",
                            ["优秀", "完美", "出色", "一流", "极佳", "卓越"])
        )
        trust_penalty = self.config.get("anti_sycophancy.layer1_lexical.trust_penalty", 2)
        high_score_threshold = self.config.get("anti_sycophancy.layer3_score.high_score_threshold", 9)
        variance_threshold = self.config.get("anti_sycophancy.layer3_score.variance_threshold", 0.5)
        evidence_fallback_score = self.config.get("anti_sycophancy.layer3_score.fallback_score", 6.0)
        security_score_threshold = self.config.get("anti_sycophancy.layer4_pattern.security_score_threshold", 7)
        security_force_score = self.config.get("anti_sycophancy.layer4_pattern.force_reduce_score", 4.0)
        evidence_min_len = self.config.get("anti_sycophancy.layer3_score.evidence_coverage_min_len", 20)
        ...
        for word in empty_praise_words:
            if word in input_text:
                ...
                if not has_evidence:
                    result["_trust_penalty"] = result.get("_trust_penalty", 0) + trust_penalty
                    result["sycophancy_warnings"].append(
                        f"[反讨好-L1] 检测到空泛赞美词「{word}」但无具体证据支撑，信任度-{trust_penalty}"
                    )

        if not result["rejection_reasons"]:
            trigger_keywords = self.config.get("anti_sycophancy.layer2_structural.trigger_keywords",
                                               ["未明确", "推断"])
            if any(kw in input_text for kw in trigger_keywords):
        ...
        for name, data in dim_scores.items():
            if data["score"] >= high_score_threshold:
                if name not in input_text or len(input_text) < evidence_min_len:
                    data["score"] = evidence_fallback_score
                    result["suggestions"].append(
                        f"[反讨好-L3] 维度「{name}」高分但证据覆盖不足，已降至 {evidence_fallback_score}"
                    )
        if len(scores) >= 2:
            ...
            if variance < variance_threshold:
                result["sycophancy_warnings"].append(
                    f"[反讨好-L3] 所有维度评分方差过低({variance:.2f}<{variance_threshold})，可能未充分评估差异，建议重新评审"
                )

        required_fields = self.config.get("anti_sycophancy.layer4_pattern.required_fields",
                                          ["security_design", "security_review"])
        security_data = dim_scores.get("安全性", dim_scores.get("合规法律", {}))
        if isinstance(security_data, dict) and security_data.get("score", 0) >= security_score_threshold:
            has_security_design = any(bool(project_info.get(f, "")) for f in required_fields)
            if not has_security_design:
                if "安全性" in dim_scores:
                    dim_scores["安全性"]["score"] = security_force_score
                elif "合规法律" in dim_scores:
                    dim_scores["合规法律"]["score"] = security_force_score
                result["sycophancy_warnings"].append(
                    f"[反讨好-L4] 安全评分≥{security_score_threshold}但未提供安全设计，已强制降至 {security_force_score}"
                )
    ...
    def _finalize(result: dict, review_type: str) -> dict:
        ...
        severity_curve = {1: 1.0, 2: 1.0, 3: 1.3, 4: 1.7}
        score_deduction_per_severity = 0.5

        penalty = 0.0
        for i in range(1, defect_count + 1):
            if i in severity_curve:
                penalty += severity_curve[i]
            else:
                penalty += 2.0 + (i - 5) * 0.5

        score_after_penalty = total_score - penalty * score_deduction_per_severity
        if penalty > 0:
            result["suggestions"].append(
        ...
        score_ceiling_by_defects = {0: 10.0, 1: 10.0, 2: 8.5, 3: 7.0}
        cap = score_ceiling_by_defects.get(defect_count, 6.0)
    ...
    def _finalize_adversarial(result: dict, project_info: dict) -> dict:
        ...
        pass_probability_cap = 0.20
        if len(kill_factors) >= 2:
            pass_probability = min(pass_probability_cap, max(0.0, total_score / 10))
        else:
    ...
    def _review_technical(self, info: dict) -> dict:
        ...
        tc = self.config.get("reviewer_rules.thresholds.test_coverage",
                             {"excellent": 80, "good": 60, "poor": 30})
        tl = self.config.get("reviewer_rules.thresholds.text_length",
                             {"detailed": 50, "moderate": 30, "brief": 20})
        dc = self.config.get("reviewer_rules.thresholds.dependency_count",
                             {"optimal": 10, "max_acceptable": 20})
        ...
        elif len(architecture) < tl["brief"]:
            arch_score = 4.0
            defect_count += 1
            suggestions.append("架构描述过于简略，建议补充详细设计")
        elif len(architecture) < tl["detailed"]:
            arch_score = 7.0
        else:
            arch_score = 9.0
        ...
        if tests >= tc["excellent"]:
            ...
            defect_count += 1
        if tests < tc["good"]:
            suggestions.append(f"测试覆盖率当前{tests}%，建议提升至{tc['excellent']}%以上")
        ...
        elif len(dependencies) <= dc["optimal"]:
            dep_score = 9.0
        elif len(dependencies) <= dc["max_acceptable"]:
            dep_score = 7.0
        else:
            dep_score = 4.0
            defect_count += 1
    ...
    def _review_investment(self, info: dict) -> dict:
        ...
        tl = self.config.get("reviewer_rules.thresholds.text_length",
                             {"detailed": 50, "moderate": 30, "brief": 20})
        budget_threshold = self.config.get("reviewer_rules.thresholds.budget", 100000)
    ...
    def _review_product(self, info: dict) -> dict:
        ...
        elif len(mvp_scope) < tl["moderate"]:
            mvp_score = 5.0
        else:
            mvp_score = 8.0
    ...
    def _get_weakness_chain_map(self) -> dict:
        default_map = {
            "无安全设计": {
                "layer1": "可能存在数据泄露风险",
                "layer2": "合规审计不通过，面临罚款",
        ...
        config_map = self.config.get("adversarial_rules.weakness_chain_map", None) if self.config else None
        return config_map if config_map else default_map

    def _get_question_bank(self) -> dict:
        default_bank = {
            "架构文档缺失": [
        ...
        config_bank = self.config.get("adversarial_rules.question_bank", None) if self.config else None
        return config_bank if config_bank else default_bank
    ...
    def _review_adversarial(self, info: dict) -> dict:
        ...
        scoring = self.config.get("adversarial_rules.scoring",
                                  {"base_score": 35, "ceiling": 75, "floor": 5})
        score = scoring["base_score"]
        ceiling = scoring["ceiling"]
        floor_val = scoring["floor"]

        input_text = " ".join(str(v) for v in info.values() if isinstance(v, str))

        unmarked_fields = self.config.get(
            "adversarial_rules.deduction_rules.unmarked_field.fields",
            ["architecture", "tech_stack", "security_design", "competitor_analysis", "cost_estimate", "exit_strategy"])
        unmarked_deduction = self.config.get(
            "adversarial_rules.deduction_rules.unmarked_field.deduction", 20)
        unmarked_keyword = self.config.get(
            "adversarial_rules.deduction_rules.unmarked_field.trigger_keyword", "未明确")

        for field_name in unmarked_fields:
            val = info.get(field_name, "")
            if isinstance(val, str) and unmarked_keyword in val:
                score -= unmarked_deduction
                defect_count += 1
                identified_weaknesses.append(f"{field_name}未明确")
                rejection_reasons.append(f"{field_name}信息未明确")

        inferred_fields = self.config.get(
            "adversarial_rules.deduction_rules.inferred_field.fields",
            ["architecture", "tech_stack", "security_design", "competitor_analysis", "cost_estimate", "user_validation"])
        inferred_deduction = self.config.get(
            "adversarial_rules.deduction_rules.inferred_field.deduction", 10)
        inferred_keyword = self.config.get(
            "adversarial_rules.deduction_rules.inferred_field.trigger_keyword", "推断")

        for field_name in inferred_fields:
            val = info.get(field_name, "")
            if isinstance(val, str) and inferred_keyword in val:
                score -= inferred_deduction
                defect_count += 1
        ...
        if not architecture or not has_arch_diagram:
            arch_deduction = self.config.get(
                "adversarial_rules.deduction_rules.missing_architecture_diagram.deduction", 25)
            arch_dim_penalty = self.config.get(
                "adversarial_rules.deduction_rules.missing_architecture_diagram.dimension_penalty", 2)
            score -= arch_deduction
            defect_count += 1
            identified_weaknesses.append("架构文档缺失")
            rejection_reasons.append("缺乏架构设计文档")
            suggestions.append("补充系统架构图及技术设计文档")
            tech_fragility = max(1, tech_fragility - arch_dim_penalty)
        ...
        if not competitor_analysis and not info.get("competitor_analysis_text", ""):
            comp_deduction = self.config.get(
                "adversarial_rules.deduction_rules.missing_competitor_analysis.deduction", 20)
            comp_dim_penalty = self.config.get(
                "adversarial_rules.deduction_rules.missing_competitor_analysis.dimension_penalty", 2)
            score -= comp_deduction
            defect_count += 1
            identified_weaknesses.append("无竞品分析")
            rejection_reasons.append("未提供竞品分析")
            suggestions.append("补充竞品对比和差异化定位")
            biz_risk = max(1, biz_risk - comp_dim_penalty)
        ...
        if not security_design and not security_review:
            sec_deduction = self.config.get(
                "adversarial_rules.deduction_rules.missing_security_design.deduction", 20)
            sec_dim_penalty = self.config.get(
                "adversarial_rules.deduction_rules.missing_security_design.dimension_penalty", 2)
            score -= sec_deduction
            defect_count += 1
            identified_weaknesses.append("无安全设计")
            rejection_reasons.append("缺乏安全设计")
            suggestions.append("补充安全架构设计和威胁建模")
            compliance = max(1, compliance - sec_dim_penalty)

        user_validation = info.get("user_validation", False)
        user_persona = info.get("user_persona", "")
        if not user_validation and not user_persona:
            uv_deduction = self.config.get(
                "adversarial_rules.deduction_rules.missing_user_validation.deduction", 15)
            uv_dim_penalty = self.config.get(
                "adversarial_rules.deduction_rules.missing_user_validation.dimension_penalty", 1)
            score -= uv_deduction
            defect_count += 1
            identified_weaknesses.append("无用户验证")
            suggestions.append("进行用户调研和需求验证")
            biz_risk = max(1, biz_risk - uv_dim_penalty)
        ...
        if not tech_stack or not has_rationale:
            tr_deduction = self.config.get(
                "adversarial_rules.deduction_rules.missing_tech_rationale.deduction", 15)
            tr_dim_penalty = self.config.get(
                "adversarial_rules.deduction_rules.missing_tech_rationale.dimension_penalty", 1)
            score -= tr_deduction
            defect_count += 1
            identified_weaknesses.append("无技术选型依据")
            suggestions.append("说明技术选型的权衡和依据")
            tech_fragility = max(1, tech_fragility - tr_dim_penalty)

        quantified_fields = self.config.get(
            "adversarial_rules.deduction_rules.missing_quantified_metrics.fields",
            ["roi_estimate", "budget", "market_size", "success_metrics", "timeline"])
        deduction_per_missing = self.config.get(
            "adversarial_rules.deduction_rules.missing_quantified_metrics.deduction_per_missing", 10)
        missing_metrics = 0
        ...
        if missing_metrics > 0:
            score -= missing_metrics * deduction_per_missing
        ...
        if not cost_estimate:
            ce_dim_penalty = self.config.get(
                "adversarial_rules.deduction_rules.missing_cost_estimate.dimension_penalty", 1)
            defect_count += 1
            identified_weaknesses.append("无成本估算")
            rejection_reasons.append("缺乏成本估算")
            suggestions.append("提供详细的项目成本估算")
            biz_risk = max(1, biz_risk - ce_dim_penalty)

        exit_strategy = info.get("exit_strategy", "")
        if not exit_strategy:
            es_dim_penalty = self.config.get(
                "adversarial_rules.deduction_rules.missing_exit_strategy.dimension_penalty", 1)
            defect_count += 1
            identified_weaknesses.append("无退出策略")
            suggestions.append("制定项目退出策略和降级方案")
            biz_risk = max(1, biz_risk - es_dim_penalty)

        team = info.get("team_background", "")
        if not team:
            tb_dim_penalty = self.config.get(
                "adversarial_rules.deduction_rules.missing_team_background.dimension_penalty", 2)
            defect_count += 1
            identified_weaknesses.append("团队背景不明")
            suggestions.append("补充团队技术背景和历史项目经验")
            exec_capability = max(1, exec_capability - tb_dim_penalty)

        test_coverage = info.get("test_coverage", 0)
        tc_threshold = self.config.get(
            "adversarial_rules.deduction_rules.missing_test_coverage.threshold", 30)
        if not test_coverage or (isinstance(test_coverage, (int, float)) and test_coverage < tc_threshold):
            tc_dim_penalty = self.config.get(
                "adversarial_rules.deduction_rules.missing_test_coverage.dimension_penalty", 2)
            defect_count += 1
            identified_weaknesses.append("无测试覆盖")
            suggestions.append("补充测试策略和覆盖率目标")
            tech_fragility = max(1, tech_fragility - tc_dim_penalty)
    ...
    def _generate_hostile_questions(self, weaknesses: List[str], info: dict) -> List[str]:
        ...
        min_count = self.config.get("adversarial_rules.hostile_questions.min_count", 5)
        max_count = self.config.get("adversarial_rules.hostile_questions.max_count", 8)
        fallback = self.config.get("adversarial_rules.hostile_questions.fallback_questions", [
        ...
        return questions[:max_count] if len(questions) > max_count else questions

    def _generate_weakness_chain(self, weaknesses: List[str]) -> List[dict]:
        """Generate weakness chains: original defect → layer1 → layer2."""
        chains: List[dict] = []
        chain_map = self._get_weakness_chain_map()
        for weakness in weaknesses:
            for key, chain_data in chain_map.items():
                if key in weakness:
                    chains.append({
                        "original_defect": key,
    ...
    def _generate_kill_factors(self, weakness_chain: List[dict], weaknesses: List[str]) -> List[str]:
        ...
        if weakness_chain:
            severity_keywords = self.config.get("adversarial_rules.kill_factors.severity_keywords",
                                                ["数据泄露", "合规审计", "罚款", "资金链断裂", "无人使用"])
            prefix_severe = self.config.get("adversarial_rules.kill_factors.prefix_severe", "【致命】")
            prefix_fallback = self.config.get("adversarial_rules.kill_factors.prefix_fallback", "【严重】")

            for chain in weakness_chain:
                for kw in severity_keywords:
                    if kw in chain["layer2_inferred"] or kw in chain["layer1_inferred"]:
                        kill_factors.append(
                            f"{prefix_severe}{chain['original_defect']} → {chain['layer1_inferred']} → {chain['layer2_inferred']}"
                        )
                        break
            ...
            if not kill_factors and weakness_chain:
                ...
                kill_factors.append(
                    f"{prefix_fallback}{chain['original_defect']} → {chain['layer1_inferred']} → {chain['layer2_inferred']}"
                )
