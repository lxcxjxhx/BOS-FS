...
Anti-sycophancy: Ensures knowledge extraction is concise and non-redundant.
  - Single chapter summary ≤ 30 lines
  - Redundancy detection with >80% similarity threshold
  - Concise reports instead of verbatim copying
"""
...
class BookIngestor:
    ...
    # Density control constants (anti-sycophancy)
    MAX_LINES_PER_CHAPTER = 30
    ...
    def _process_book(self, text: str, book_name: str, output_dir: str) -> Dict[str, List[str]]:
        ...
        # Track existing content for redundancy detection
        ...
        for chapter in chapters:
            ...
            for layer_name, layer_insights in layer_map.items():
                ...
                # Redundancy check: skip if >80% similar to existing content
                chapter_summary = "\n".join(
                    item for items in layer_insights.values() for item in items
                )
                if self._check_redundancy(chapter_summary, existing_content[layer_name]):
                    continue
                ...
                # Density enforcement: truncate to MAX_LINES_PER_CHAPTER
                content_lines = content.split("\n")
                content_lines = self._enforce_density(content_lines)
                content = "\n".join(content_lines)
                ...
                existing_content[layer_name].append(chapter_summary)
    ...
    def _text_similarity(text_a: str, text_b: str) -> float:
        """Calculate Jaccard similarity between two text snippets (word-level).
    ...
    def _check_redundancy(self, new_text: str, existing_texts: List[str]) -> bool:
        """Check if new_text is redundant (>80% similar) compared to existing texts.

        Returns:
            True if redundant (should be skipped), False if unique (should be kept).
        """
        for existing in existing_texts:
            if self._text_similarity(new_text, existing) > self.REDUNDANCY_SIMILARITY_THRESHOLD:
                return True
        return False

    def _enforce_density(self, lines: List[str]) -> List[str]:
        """Truncate knowledge page lines to MAX_LINES_PER_CHAPTER.

        Keeps: header + source/layer metadata + most important content.
        Truncates: excess items in each section.
        """
        max_lines = self.MAX_LINES_PER_CHAPTER
        if len(lines) <= max_lines:
            return lines

        # Always keep first 5 lines (header + metadata)
        header_lines = lines[:5]
        content_lines = lines[5:]

        # Truncate content proportionally
        budget = max_lines - len(header_lines) - 2  # -2 for footer
        if budget <= 0:
            return header_lines + ["[内容超出密度限制，已截断]"]

        return header_lines + content_lines[:budget] + ["", f"[密度控制: 原始 {len(lines)} 行 → 截断至 {max_lines} 行]"]
