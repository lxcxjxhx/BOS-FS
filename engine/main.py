...
def main():
    ...
    features = ["多模型调度", "自动测试", "CI/CD"]
    r = mapper.map_features(features)
    for item in r["features"]:
        print(f"  {item['feature']} → {item['outcome']}")
...
"""
    ...
    goal = refiner.refine("企业研发团队，交付效率低，缺标准化。方案：自动化交付平台。目标：提升50%。")
    r = simulator.simulate("technical", goal)
    print(f"  [Technical] Pass Probability: {r['pass_probability']:.0%}")

    print("\n=== Adversarial Review Demo ===")
    r = simulator.simulate("adversarial", goal)
    print(f"  [Adversarial] Pass Probability: {r['pass_probability']:.0%}")
    print(f"  Hostile Questions: {len(r['hostile_questions'])}")
    print(f"  Kill Factors: {len(r['kill_factors'])}")
    print(f"  Weakness Chains: {len(r['weakness_chain'])}")
