...
| 平台 | 加载方式 | 文件 |
|------|----------|------|
| Cursor | `@skills/bos-fs-pipeline/SKILL.md` | 根目录 `.cursorrules` 自动加载 |
| Trae | `/skill bos-fs/pipeline` | `.trae/skills/bos-fs/` 自动加载 |
| OpenHands | 引用 skills/ 目录 | 对应 Skill .md 文件 |
| Claude Code | `@skills/bos-fs-pipeline/SKILL.md` | Skill .md 文件或 Custom Instructions |
| OpenDevin | 配置 skills 路径 | skills/bos-fs-pipeline/SKILL.md |
| Copilot | `#file:skills/bos-fs-pipeline/SKILL.md` | 自定义指令或 .github/copilot-instructions.md |
| 通用 LLM | 读取 .md 作为 system prompt | skills/bos-fs-pipeline/SKILL.md |
...
```
# 方式一：直接引用
@skills/bos-fs-pipeline/SKILL.md
...
```python
# Step 1: 读取 Skill 提示词
system_prompt = read_file("skills/goal-refiner/SKILL.md")
...
def load_skill(name):
    with open(f"skills/{name}/SKILL.md") as f: return f.read()

def bosfs_pipeline(desc, readme=""):
    goal = call_llm(load_skill("goal-refiner"), desc)
    outcomes = call_llm(load_skill("outcome-mapper"), goal)
    new_readme = call_llm(load_skill("readme-refactor"), readme)
    review = call_llm(load_skill("reviewer-simulator"), {**goal, **outcomes})
    bundle = call_llm(load_skill("submission-builder"), {**goal, "readme": new_readme})
...
def bosfs_pipeline(project_description):
    with open("skills/bos-fs-pipeline/SKILL.md") as f:
...
{
  "skills": [
    {"name": "bos-fs", "path": "skills/", "pipeline": "bos-fs-pipeline/SKILL.md"}
  ]
}
```
...
```
#file:skills/bos-fs-pipeline/SKILL.md
...
- Load skills/bos-fs-pipeline/SKILL.md for full workflow
