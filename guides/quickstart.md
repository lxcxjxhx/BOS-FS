...
# 通用 LLM
读取 skills/bos-fs-pipeline/SKILL.md 作为 system prompt
````
...
```
加载: skills/goal-refiner/SKILL.md
输入: "做了个AI工作流系统"
...
```
加载: skills/reviewer-simulator/SKILL.md
...
```
加载: skills/readme-refactor/SKILL.md
输入: 原始 README
...
```
加载: skills/outcome-mapper/SKILL.md
输入: "多模型调度"
...
```
加载: skills/submission-builder/SKILL.md
...
```
加载: skills/reject-analyzer/SKILL.md
...
Step 4 → Review:    五类评审模拟，获取通过概率和建议
...
**Q: 如何单独使用某个 Skill？**
A: 直接加载对应的 `skills/<skill-name>/SKILL.md` 文件，按文件中的输入格式提供数据。

**Q: 评审通过率多少算合格？**
A: 技术评审 > 70% 为合格，50–70% 需要改进，< 50% 需要大幅修改。

**Q: 信任声明是什么？**
A: 基于权威框架（OWASP / ISO 25010 / DORA 等）的技术可信度声明，详见 [knowledge/governance/trust_framework.md](../knowledge/governance/trust_framework.md)。
