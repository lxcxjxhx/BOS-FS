...
│  ┌───────────────────────────────────────────────────┐   │
│  │                Skill Layer (技能层)                │   │
│  │  goal-refiner       reviewer-simulator            │   │
│  │  readme-refactor    outcome-mapper                │   │
│  │  submission-builder reject-analyzer               │   │
...
**核心论点**: 软件架构由结构+架构特性（-ilities）+架构决策+设计原则四部分组成。架构师的核心职责是做决策并持续分析架构健康度。
**关键概念**: 架构四要素（Structure/Characteristics/Decisions/Principles）、架构师八项期望、架构 vs 设计区别、上下文敏感性
**实践方法**: 用"-ilities"定义成功标准；架构决策指导而非代替技术选择；持续分析架构生命力
**BOS-FS 映射**: Pipeline 即"架构决策执行工具"——每个 Stage 是架构决策的体现（Goal Refiner→Outcome Mapper→Reviewer）。架构特性（可插拔性/可维护性）通过 Stage 接口协议保障。

### Chapter 2: Architectural Thinking
**核心论点**: 架构师需要"缩放"能力——从宏观（系统上下文）到微观（代码细节）之间自由切换。架构思维是理解全局与局部的关系。
**关键概念**: 缩放思维（Zoom-in/Zoom-out）、系统思维、模块思维、技术广度优于深度
**实践方法**: 练习从不同抽象层级分析问题；技术广度覆盖10+技术栈的优缺点；理解业务领域语言
**BOS-FS 映射**: BOS-FS 设计体现缩放思维——宏观（Pipeline 五类评审）到微观（每个 Stage 的 Prompt）。知识体系要求广度（架构/安全/交付/组织）而非深度。

### Chapter 3: Modularity Fundamentals
**核心论点**: 模块化是架构设计的基石。好模块有高内聚、低耦合、清晰接口。模块化架构支持独立开发、测试和部署。
**关键概念**: 内聚（Cohesion）、耦合（Coupling）、模块边界、信息隐藏、接口契约
**实践方法**: 按"变化频率"分组代码；定义清晰模块接口；减少模块间共享数据；用依赖倒置降低耦合
**BOS-FS 映射**: BOS-FS 的 Stage 设计是模块化典范——每个 Stage 独立开发/测试/替换，通过 JSON 数据总线通信。Skill 是可插拔模块，遵循接口契约。

### Chapter 4: Architecture Styles
**核心论点**: 架构风格（微服务/事件驱动/微内核/分层等）各有适用场景和权衡。选择架构风格需要基于架构特性需求。
**关键概念**: 微内核架构（BOS-FS 当前）、事件驱动架构、分层架构、微服务架构、空间架构
**实践方法**: 根据架构特性优先级选择风格；混合风格是常态；风格演进比一步到位更可行
**BOS-FS 映射**: BOS-FS 当前采用微内核架构（核心+插件/Skill）。Pipeline 体现事件驱动风格（Stage 间数据流）。未来可能向事件驱动演进。

### Chapter 5: Layered Architecture
**核心论点**: 分层架构是最基础的架构风格，通过层间隔离实现关注点分离。但分层可能导致"下沉式"依赖和性能问题。
**关键概念**: 封闭分层 vs 开放分层、层间依赖方向、下沉式问题、层粒度选择
**实践方法**: 封闭分层确保变更隔离；避免层跳过（presentation 直接调 persistence）；合理定义层边界
**BOS-FS 映射**: BOS-FS 五层架构（Intent/Runtime/Execution/Governance/Skill）是分层架构的应用——每层职责明确，依赖方向向上。

### Chapter 6: Microkernel Architecture
**核心论点**: 微内核架构（插件架构）核心系统最小化，功能通过插件扩展。适合需要持续扩展和定制化的产品。
**关键概念**: 核心系统、插件、插件注册表、插件契约、插件通信
**实践方法**: 核心系统保持稳定；插件通过注册表发现；定义清晰的插件接口；插件间通过核心协调
**BOS-FS 映射**: BOS-FS 是典型的微内核架构——Core 是最小化内核，Skill 是插件，通过 Skill Manifest 注册。Custom Stage 机制是插件扩展能力的体现。

### Chapter 7: Event-Driven Architecture
**核心论点**: 事件驱动架构通过事件流解耦组件，支持异步、可扩展的系统。但增加了复杂性和调试难度。
**关键概念**: 事件流、事件源、消费者、中介（Broker）、事件溯源
**实践方法**: 定义清晰的事件契约；处理事件乱序和重复；建立事件监控；选择同步 vs 异步模式
**BOS-FS 映射**: Pipeline Stage 间数据流是"事件流"的简化版本。未来可向真正的事件驱动架构演进——Stage 间通过事件总线通信。

### Chapter 8: Space-Based Architecture
**核心论点**: 空间架构通过分布式内存网格消除数据库瓶颈，实现高可扩展性。适合高并发、低延迟场景。
**关键概念**: 内存网格、处理单元、数据网格、虚拟中间件
**实践方法**: 将热数据放入内存网格；使用处理单元实现计算扩展；避免共享数据库
**BOS-FS 映射**: 当前 BOS-FS 不需要空间架构（本地运行）。但如果演进为 SaaS 服务，Pipeline 执行引擎可能需要空间架构支持高并发。

### Chapter 9: Orchestration-Driven SOA
**核心论点**: 编排式 SOA 通过中央编排器协调服务交互。适合复杂业务流程，但编排器可能成为瓶颈。
**关键概念**: 编排器（Orchestrator）、服务契约、业务流程、服务复用
**实践方法**: 编排器保持轻量；服务契约稳定；避免编排器逻辑膨胀
**BOS-FS 映射**: Pipeline 编排器即 BOS-FS Core——它编排各 Stage 执行顺序。Core 应保持轻量，将复杂逻辑委托给 Stage。

### Chapter 10: Microservices Architecture
**核心论点**: 微服务架构将系统拆分为独立部署的小服务。每个服务有独立数据库和部署流水线。适合大型团队和持续交付。
**关键概念**: 服务边界、独立部署、API 网关、服务发现、分布式事务
**实践方法**: 按业务能力拆分服务；每个服务独立数据库；建立 API 网关；处理分布式事务
**BOS-FS 映射**: BOS-FS 当前是单进程架构，但 Pipeline 设计理念与微服务一致——每个 Stage 是"服务"，可独立演进。未来可拆分为微服务。

### Chapter 11: Choosing Architecture Styles
**核心论点**: 架构风格选择是基于架构特性的权衡过程。没有"最佳"风格，只有"最适合"的风格。
**关键概念**: 架构特性优先级矩阵、风格对比表、混合风格策略、演进路径
**实践方法**: 确定 Top 3 架构特性；用对比表评估风格；选择混合风格组合；规划演进路径
**BOS-FS 映射**: BOS-FS 架构选择：微内核（可插拔性）+ 分层（关注点分离）+ 事件驱动（Stage 间通信）。选择过程遵循"架构特性优先"原则。

### Chapter 12: Architecture Decisions
**核心论点**: 架构决策是架构师的核心产出。好决策有清晰的理由、影响范围和生效日期。决策需要文档化和定期回顾。
**关键概念**: ADR（Architecture Decision Record）、决策模板、决策回溯、决策变更流程
**实践方法**: 每个决策记录背景/选项/决策/后果；定期回顾决策是否仍然有效；建立决策变更流程
**BOS-FS 映射**: BOS-FS 的架构决策应记录为 ADR——为什么选择微内核、为什么用 Markdown 知识文件、为什么用 JSON 数据总线。Pipeline 配置即"活的决策"。

### Chapter 13: Negotiation and Leadership Skills
**核心论点**: 架构师的影响力来自领导力而非职权。架构决策需要谈判和说服，而非强制执行。
**关键概念**: 利益相关者分析、谈判策略、影响力模型、冲突解决
**实践方法**: 理解各方利益诉求；用数据支持决策；建立共识而非强制；处理技术冲突
**BOS-FS 映射**: BOS-FS 推广需要"谈判"——说服团队采用 Pipeline 而非绕过。Pipeline 五类评审报告是"数据支持决策"的工具。
