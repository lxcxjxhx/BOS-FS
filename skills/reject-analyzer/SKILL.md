...
```json
{
  ...
  "properties": {
    ...
    "checklist_results": {
      "type": "object",
      "properties": {
        "clear_problem": { "type": "boolean" },
        "value_proposition": { "type": "boolean" },
        "differentiation": { "type": "boolean" },
        "documentation": { "type": "boolean" },
        "security_privacy": { "type": "boolean" },
    ...
    "multi_patterns": {
      ...
      "items": {
        "type": "object",
        "properties": {
          "pattern_name": { "type": "string", "minLength": 1, "description": "匹配的拒绝模式名称" },
          "confidence": { "type": "number", "minimum": 0.0, "maximum": 1.0, "description": "该模式匹配置信度" }
        },
        "required": ["pattern_name", "confidence"],
        "additionalProperties": false
      },
      "minItems": 1,
      "description": "多模式匹配结果（当一条拒绝涉及多个模式时）"
    }
  },
...
| multi_patterns | array | ❌ | 见下方 | 多模式匹配结果（当一条拒绝涉及多个模式时） |

### checklist_results 子字段
| 子字段 | 类型 | 必填 | 说明 |
|--------|------|------|------|
| clear_problem | boolean | ✅ | 问题描述是否清晰 |
| value_proposition | boolean | ✅ | 是否有价值主张 |
| differentiation | boolean | ✅ | 是否有差异化 |
| documentation | boolean | ✅ | 文档是否完整 |
| security_privacy | boolean | ✅ | 安全/隐私是否覆盖 |
| compliance | boolean | ✅ | 合规是否明确 |
| roadmap | boolean | ✅ | 是否有路线图 |
...
- **checklist_results 完整性**: 7个子字段（clear_problem/value_proposition/differentiation/documentation/security_privacy/compliance/roadmap）缺一不可，均为 boolean 类型
- **必填字段**: 顶层7个字段缺一不可
...
Output: {"real_issue":"表达太底层，用户价值弱","fixable_items":["技术描述转用户价值","增加场景描述","补充量化指标"],"resubmit_suggestion":"从'交付工具'重新定位为'交付平台'，强调自动化和效率提升","matched_pattern":"表达太底层","confidence":0.85,"checklist_results":{"clear_problem":false,"value_proposition":false,"differentiation":true,"documentation":true,"security_privacy":true,"compliance":true,"roadmap":true},"root_cause_tree":{"surface":"看不出用户价值","primary":"技术描述替代了价值主张，评审无法理解谁受益、受益多少","secondary":"Feature→Capability→Outcome 转换链断裂，停留在技术实现层","fix_path":"对每个功能追问'让用户做什么？'和'带来什么收益？'，将技术特性翻译为用户能力"}}

Input: "又一个AI Agent工具"
Output: {"real_issue":"未突出独特价值主张","fixable_items":["明确与竞品差异","突出核心定位","补充竞品对比表"],"resubmit_suggestion":"强调'让成果被接受'而非'成果生成'，定位为Acceptance Engineering","matched_pattern":"差异化不足","confidence":0.9,"checklist_results":{"clear_problem":true,"value_proposition":true,"differentiation":false,"documentation":true,"security_privacy":true,"compliance":true,"roadmap":true},"root_cause_tree":{"surface":"又一个XX工具","primary":"未展示与现有方案的实质性差异，评审视为同质化产品","secondary":"缺乏竞品分析，或差异化定位停留在功能列表而非价值主张","fix_path":"选择2-3个直接竞品，从'解决什么问题不同'和'为谁解决'两个维度建立差异化矩阵"}}
```
...
- [ ] checklist_results的7个子字段（clear_problem/value_proposition/differentiation/documentation/security_privacy/compliance/roadmap）是否全部填写？
