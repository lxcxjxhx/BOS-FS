...
```json
{
  ...
  "properties": {
    ...
    "scores": {
      ...
      "description": "各维度评分（含trust）。必须包含对应评审类型的特定维度键：technical→架构设计/代码质量/安全性/性能可扩展/trust；investment→市场规模/竞争优势/团队能力/商业模式/风险因素/trust；product→需求匹配/差异化/用户体验/功能规划/路线图/trust；opensource→社区价值/文档完整性/可维护性/许可证合规/社区治理/trust；adversarial→技术脆弱性/商业风险/执行能力/合规法律/致命缺陷/trust"
    },
    "weighted_score": {
      "type": "number",
      "minimum": 0.0,
