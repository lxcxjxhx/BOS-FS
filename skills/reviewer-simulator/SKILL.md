...
> Config: [reviewer_rules.json](../../engine/config/defaults/reviewer_rules.json)
> Flexibility: [flexibility_principles.md](../knowledge/flexibility_principles.md)
...
| 模式 | 说明 | 适用场景 |
|------|------|----------|
| **完整评审（Full Review）** | 评审所有维度（当前默认行为） | 正式评审/客户交付评审/最终决策 |
| **轻量评审（Light Review）** | 仅评审核心维度，权重重新分配 | 快速预审/内部评审/日常迭代 |

### 轻量评审模式配置
仅启用核心维度，核心维度权重归一化至100%：

| 评审类型 | 核心维度（轻量模式） | 权重重新分配 |
|----------|----------------------|--------------|
| technical | 架构设计、代码质量、安全性 | 34% / 33% / 33% |
| investment | 市场规模、竞争优势、商业模式 | 34% / 33% / 33% |
| product | 需求匹配、差异化、用户体验 | 34% / 33% / 33% |
| opensource | 社区价值、文档完整性、许可证合规 | 34% / 33% / 33% |
| adversarial | 技术脆弱性、商业风险、致命缺陷 | 34% / 33% / 33% |

> 轻量评审模式下 `trust` 维度仍然评估，但不计入加权总分。

### 完整评审模式配置
评审所有维度，使用下表中的默认权重（当前行为）。

### 模式选择规则
- 评审类型含 `adversarial` 时默认使用**完整评审**模式
- 用户明确要求"快速评审"/"预审"/"light"时切换至**轻量评审**模式
- 正式评审/客户交付/最终决策时使用**完整评审**模式

## Scoring Dimensions

> **权重说明**: 以下标注的权重为**默认权重**，可通过 `engine/config/defaults/reviewer_rules.json` 自定义。场景配置可覆盖默认权重。详见 [flexibility_principles.md](../knowledge/flexibility_principles.md)。
...
> **权重说明**: 所有维度权重为默认值，可按需通过配置文件调整。支持轻量评审/完整评审两种模式，模式选择影响参与评分的维度集合及权重分配。

### 缺陷加权非线性递减
当缺陷数 ≥ 2 时，pass_probability 按公式计算：
...
## Output
```json
{"review_type":"<type>","review_mode":"<full|light>","pass_probability":<5-95>,"scores":{"维度1":<0-10>,"...":<0-10>,"trust":<0-10>},"weighted_score":<0.0-10.0>,"rejection_reasons":["string"],"suggestions":["string"],"sycophancy_warnings":["string"],"hostile_questions":["string"],"weakness_chain":[{"original":"<string>","inferred_layer1":"<string>","inferred_layer2":"<string>"}],"kill_factors":["string"]}
```
评分：8-10充分；5-7基本；2-4简略；0-1缺失
pass_probability 范围：普通模式 10–95，敌对模式 5–75
review_mode: 评审模式，`full`（完整评审）或 `light`（轻量评审）
...
```json
{
  ...
  "properties": {
    ...
    "review_mode": {
      "type": "string",
      "enum": ["full", "light"],
      "description": "评审模式。full=完整评审（所有维度），light=轻量评审（仅核心维度）"
    },
    "pass_probability": {
      "type": "integer",
      "minimum": 5,
      "maximum": 95,
      "description": "通过概率（百分比）。普通模式: 10–95, 敌对模式: 5–75"
    },
    "scores": {
      "type": "object",
      ...
      "description": "各维度评分（含trust）。必须包含对应评审类型的特定维度键：technical→架构设计/代码质量/安全性/性能可扩展/trust；investment→市场规模/竞争优势/团队能力/商业模式/风险因素/trust；product→需求匹配/差异化/用户体验/功能规划/路线图/trust；opensource→社区价值/文档完整性/可维护性/许可证合规/社区治理/trust；adversarial→技术脆弱性/商业风险/执行能力/合规法律/致命缺陷/trust。轻量评审模式下仅输出核心维度+trust。"
    },
    "weighted_score": {
      "type": "number",
      "minimum": 0.0,
      "maximum": 10.0,
      "description": "加权总分（按当前模式权重计算）"
    },
    "rejection_reasons": {
      "type": "array",
  ...
  "required": ["review_type", "review_mode", "pass_probability", "scores", "weighted_score", "rejection_reasons", "suggestions", "sycophancy_warnings", "hostile_questions", "weakness_chain", "kill_factors"],
  "additionalProperties": false
}
```
...
| review_mode | string | ✅ | full / light | 评审模式，full=完整评审，light=轻量评审 |
| pass_probability | integer | ✅ | 5–95 | 通过概率百分比，普通模式10-95，敌对模式5-75 |
| scores | object | ✅ | 每维度 0–10 | 按评审类型对应维度评分，必须含 `trust`。轻量评审模式下仅含核心维度 |
| weighted_score | number | ✅ | 0.0–10.0 | 加权总分，保留1位小数，按当前模式权重计算 |
...
- **轻量评审验证**: `review_mode` 为 `light` 时，`scores` 仅包含对应评审类型的核心维度 + `trust`
...
Output: {"review_type":"technical","review_mode":"full","pass_probability":65,"scores":{"架构设计":5,"代码质量":5,"安全性":4,"性能可扩展":5,"trust":6},"weighted_score":4.8,"rejection_reasons":["缺少架构文档","未说明技术选型","安全性不足"],"suggestions":["补充架构设计","明确技术选型","增加安全评估","量化目标"]}

Input: {"persona":"开发者","problem":"未明确","solution":"一个Python工具库","outcome":"未明确"}
Output: {"review_type":"opensource","review_mode":"light","pass_probability":25,"scores":{"社区价值":2,"文档完整性":1,"许可证合规":1,"trust":2},"weighted_score":1.3,"rejection_reasons":["社区价值未明确","缺少README","许可证未说明"],"suggestions":["明确目标和用户","编写README","选择许可证","添加示例"]}
```

### Adversarial 模式示例
```
Input: {"persona":"开发者","problem":"多模型切换成本高","solution":"AI工作流系统，支持多模型调度","outcome":"提升开发效率"}
Output: {"review_type":"adversarial","review_mode":"full","pass_probability":30,"scores":{"技术脆弱性":3,"商业风险":4,"执行能力":2,"合规法律":3,"致命缺陷":2,"trust":3},"weighted_score":2.8,"rejection_reasons":["无架构图，技术债可能严重","无竞品分析，市场定位不清晰","无安全设计，数据泄露风险","无用户验证数据，需求可能是伪需求","团队背景不明，执行能力存疑","量化指标缺失，'提升效率'不可验证"],"suggestions":["补充架构设计文档","进行竞品对比分析","增加安全设计方案","提供用户调研数据","量化效率提升指标"],"sycophancy_warnings":[],"hostile_questions":["为什么现有AI工作流工具（如LangChain/CrewAI）不能解决这个问题？","如果OpenAI/Anthropic推出原生工作流功能，你的护城河是什么？","有什么证据证明开发者愿意为此付费？","你的方案中最大的技术假设是什么？如何验证？"],"weakness_chain":[{"original":"无架构图","inferred_layer1":"技术债可能严重","inferred_layer2":"维护成本不可控"},{"original":"无竞品分析","inferred_layer1":"市场定位不清晰","inferred_layer2":"商业化可行性存疑"}],"kill_factors":["无用户验证数据→需求可能是伪需求→产品可能无人使用"]}
```

## Anti-Patterns — 五层反谄媚防护
...
| 轻量评审模式下检测到核心维度以外的缺陷 | 在 rejection_reasons 中标注"（轻量评审未覆盖）"，不纳入加权计算 |

## Quality Gates
输出前自检：
1. 每个维度（含trust）是否均在0-10范围内？
2. pass_probability是否在对应模式的范围内？（普通：10-95，敌对：5-75）
3. review_mode 是否正确标识（full/light）？
4. 轻量评审模式下 scores 是否仅包含核心维度 + trust？
5. weighted_score 是否按当前模式的权重计算？
6. **Layer 1**: 是否存在无证据支撑的空泛赞美词？如有是否已降级trust分？
7. **Layer 2**: 是否存在明显缺陷但rejection_reasons为空？如有是否已生成？
8. **Layer 3**: 高分维度（9-10分）的检查项覆盖率是否≥50%？
9. **Layer 4**: 是否存在连续高分模式？是否触发严格模式？
10. 缺陷数量≥3时，是否应用了非线性递减公式？
11. 每个rejection_reason是否有对应的suggestion？
12. 是否所有维度得分完全相同（方差<0.5）？如是是否触发重新评估？
13. **敌对模式**: pass_probability是否≤75？如>75是否触发审查？
14. **敌对模式**: weakness_chain是否非空？至少包含1条弱点链？
15. **敌对模式**: kill_factors是否至少包含1个致命缺陷？
16. **敌对模式**: hostile_questions数量是否≥5？

## 权重配置系统（v0.3.1+）

本 Skill 的评分权重支持灵活配置，不再硬编码固定值。

### 默认权重文件
默认权重定义于：
```
engine/config/defaults/reviewer_rules.json
```
...
不同场景可通过场景配置文件覆盖默认权重：
```
engine/config/scenarios/<scenario_name>/reviewer_rules.json
```
...
### 自定义权重规则
- 权重为**默认值**，可按需调整
- 同一评审类型的权重总和必须为 100%（或归一化后等效）
- 轻量评审模式仅保留核心维度，权重自动归一化
- 新增维度需在 `reviewer_rules.json` 中同步定义

详见 [flexibility_principles.md](../knowledge/flexibility_principles.md)
